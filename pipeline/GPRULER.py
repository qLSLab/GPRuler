...
# dip = input('Do you want to use DIP database? yes/no ')
dip = sys.argv[1]
...
if analysis == '1':
    ...
    if testModel == '1':
        ...
        ## Recon 3
        dfrxns2Genes = 'Recon3D_Rxns2Genes'
        dfKegg2UniprotGenesFileName = 'Recon3D_Kegg2UniprotGenes'
        regexOrgSpecific = r"([0-9]+)"
        model = 'Recon3'
        organismCode = 'hsa'
    elif testModel == '3':
...
# #############################################################
# dfData = pd.read_csv(os.path.join(OUTDIR, model + '_GenesData_' + timeStamp + '.csv'), sep = '\t', dtype = str)
...
# if dip == 'yes':
#     dfData['complex_fromDIP'] = dfData['complex_fromDIP'].fillna('[]')
#     dfData['complex_fromDIP'] = dfData['complex_fromDIP'].apply(literal_eval)
#     dfData['binary_fromDIP'] = dfData['binary_fromDIP'].fillna('[]')
#     dfData['binary_fromDIP'] = dfData['binary_fromDIP'].apply(literal_eval)
#
...
for row in dfRxnToGenes.itertuples():
    ...
    if len(lCatalystGenes) == 0:
        ...
        print('one gene\t', lCatalystGenes[0])
        if lCatalystGenes[0] != []:
            if analysis == '1':
        ...
        if analysis == '1':
            ...
            # lCatalystGenes_original = lCatalystGenes.copy()
            ...
            print('lCatalystGenes_original con piu di 1 gene\t', len(lCatalystGenes_original), '\t', lCatalystGenes_original[:10])
            ...
            # lCatalystGenes = [el for item in lCatalystGenes for el in item]
            print('lCatalystGenes con piu di 1 gene\t', len(lCatalystGenes), '\t', lCatalystGenes[:10])
        if len(lCatalystGenes) == 1:
            ...
            for c in combin:
                ...
                for u0 in luniprot0:
                    ...
                    # comparison0 = [rule == luniprot0_append for rule in dfGenesRelationships['uniprotId']] # cambiato con riga dopo
                ...
                for u1 in luniprot1:
                    ...
                    # comparison1 = [rule == luniprot1_append for rule in dfGenesRelationships['uniprotId']] # cambiato con riga dopo
            ...
            if andFlag is True and orFlag is False:
                ...
                lParentheses_or = []
                allGenes = []
                print('lCombin\t', len(lCombin), '\t', lCombin[:10])
                j= 0
                for l in lCombin:
                    j+= 1
                    print('j\t', j)
                ...
                print('lParentheses_and\t', len(lParentheses_and), '\t', lParentheses_and[:10])
                print('lParentheses_or\t', len(lParentheses_or), '\t', lParentheses_or[:10])
                ...
                for i in range(1, len(lParentheses_and)):
                    ...
                    print('tmpI\t', tmpI)
                    coexist = False
                    print('Lunghezza parts\t', len(ruleParts))
                    for p in ruleParts:  # calcolo inmtersezione con ogni elemento ddella lista e se positivo allora verifico che ci sia congruenza anche con altro elemento della lista
                        print('p \t', p)
                        tmpP = p.copy()
                ...
                print('ruleParts\t', len(ruleParts), '\t', ruleParts[:10])
                ...
                if len(ruleParts) > 1:
                    ...
                    print('inside ruleParts\t', len(ruleParts))
                    for c in ruleParts:
                        print('c\t', c)
                    ...
                    print('ruleParts_remainingGenes_combs\t', len(ruleParts_remainingGenes_combs), '\t', ruleParts_remainingGenes_combs[:10], '\n')
                    rComb = 0
                    ...
                    print('orCombinations\t', len(orCombinations), '\t', orCombinations[:10], '\n')
                    oComb = 0
                    ...
                    present = True
                    orCombinations = gprL.unique(orCombinations)
                    print('orCombinations2\t', len(orCombinations), '\t', orCombinations[:10])
                    pr = 0
                    ...
                    if present == True:
                        lpartAnd = []
                        print('ruleParts_remainingGenes\t', len(ruleParts_remainingGenes))
                    ...
                    isAllOr = True
                    lRemainingOr = []
                    print('combinToEvaluate\t', len(combinToEvaluate))
                    ...
                    ruleLastVersion = ruleFinale[:]
                    print('ruleLastVersion\t', ruleLastVersion)
