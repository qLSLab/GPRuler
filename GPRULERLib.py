...
def textMiningFromUniprot(df):
    ...
    lredundancies = []
    for i in range(0, len(df)):
        funzione = df['function'][i]
        testo = df['txt_subunit'][i]
        lPossibleSubunitsFromName = []

        namesWithIsoformIndication = False
        lAllNames = df['proteinNames'][i] + df['geneNames'][i]
        for el in lAllNames:
        ...
        if "by similarity" in testo.lower():
            try:
                bysimilarity += [word for word in word_tokenize(df['txt_subunit'][i].split("By similarity")[0].split(".")[1], 'english', False) if word.isupper() and len(word) > 2]
            except:
                bysimilarity += [word for word in word_tokenize(df['txt_subunit'][i].split("By similarity")[0], 'english', False) if word.isupper() and len(word) > 2]
        ...
        for frase in lTestoSplt:
            ...
            for col in columns:
                ...
                while (orPosition != -1) and ((any(frase_before.endswith(el) for el in df[col][i]) is True) or (any(frase_after.startswith(el) for el in df[col][i]) is True)):
                    tmpIsoformList = []
                    foundWords = [el for el in df[col][i] if el in frase]
                    wordsToRemove = []
                    for f in foundWords:
                        ...
                        if len(lfraseSplt) == 2:
                            ...
                            if el0.endswith(' or') is True and col == 'geneNames':
                                ...
                                dfout0 = gprL.extractRegexFromItem(out0, r"([A-Za-z0-9-]+)")
                                lout0 = list(dfout0[0])
                            ...
                            if el1.startswith('or ') is True and col == 'geneNames':
                                ...
                                dfout1 = gprL.extractRegexFromItem(out1, r"([A-Za-z0-9-]+)")
                                lout1 = list(dfout1[0])
            ...
            for w in wordsSelection:
                ...
                for p in pos:
                    ...
                    if 'isoforms of' not in frase_sub:
                        try:
                            frase_sub_splitted = reModule.split(w, frase_sub)[1]
                        except:
        ...
        for unip in uniprotIds:
            ...
            if res != '':
                ...
                try:
                    for ref in dRes['uniprot']['entry']['dbReference']:
                        if ref['@type'] == 'DIP':
                            currentDipId.append("".join(reModule.findall("[0-9]",ref['@id'])))
                            tmpDipId.append("".join(reModule.findall("[0-9]", ref['@id'])))
                except:
                    dipId = ''
                for singleDipId in tmpDipId:
                    id_unidip = ''
                    id_binarydip = ''
                    url = 'https://dip.doe-mbi.ucla.edu/dip/Browse.cgi?PK=' + str(singleDipId) + '&MD=1'
                    response = requests.get(url, verify=False)
                    test = response.content
                    dfs = pd.read_html(test)
                    for i in range(0, len(dfs[8]['Cross Reference']['SWISSPROT'])):
                        if str(dfs[8]['Cross Reference']['SWISSPROT'][i]) != "nan":
                            currentDipComplex.append(dfs[8]['Cross Reference']['SWISSPROT'][i])

                    url = 'https://dip.doe-mbi.ucla.edu/dip/Browse.cgi?PK=' + str(singleDipId) + '&MD=0'
                    response = requests.get(url, verify=False)
                    test = response.content
                    dfs = pd.read_html(test)
                    for i in range(0, len(dfs[8]['Cross Reference']['SWISSPROT'])):
                        if str(dfs[8]['Cross Reference']['SWISSPROT'][i]) != "nan":
                            currentDipBinary.append(dfs[8]['Cross Reference']['SWISSPROT'][i])
...
def getStringData(df, gene='id_uniprot'):
    ...
    for i, row in df.iterrows():
        ...
        lStringInteractors = gprL.unique(lStringInteractors)
...
def mergeData(df):
    ...
    lTotalGenes = gprL.unique(lTotalGenes)
    ...
    for i, r in df.iterrows():
        ...
        finallDipComplex = gprL.intersect(lDipComplex, lTotalGenes)
        lDipBinary = [x for x in list(r['binary_fromDIP']) if x not in list(r['id_uniprot'])]
        finallDipBinary = gprL.intersect(lDipBinary, lTotalGenes)
        finallDipBinary = gprL.unique(finallDipBinary)
        dfinallDipBinary_names = {}
        for f in finallDipBinary:
            out = df.loc[df['uniprotId'] == f]
            for i, o in out.iterrows():
                dfinallDipBinary_names[f] = gprL.unique([o['uniprotId']] + o['proteinNames'] + o['geneNames'] + o['id_uniprot'] + o['geneName_fromKEGG'])
        ...
        lAllIsoforms = gprL.unique(lAllIsoforms)
        ...
        subunit += ldfInteract + ldfSimilarity + gprL.difference(finalStringSubs, lAllIsoforms) + finalOtherSubunits
        subunit = gprL.unique(subunit)
...
def generateOrganismSpecificRegex(keggGeneList):
    ...
    noAlphaNumeric = []
    for gene in keggGeneList:
        noAlphaNumeric += re.findall(r'\W+', gene)
